...
.filter('emptyOrInvited', function (GameStates, UserSession) {
  return function (games) {
    ...
    list = list.filter(function (game) {
      return (game.state > GameStates.opened && ((Array.isArray(game.invitations) && game.invitations.indexOf(UserSession.currentUser().uid) != -1) || !game.invitations)) || game.host.uid == UserSession.currentUser().uid
    })
